        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
              ...
              return (
                <div key={post.id} className="flex flex-col md:flex-row border border-gray-200 bg-white overflow-hidden hover:shadow-md transition-shadow duration-300">
                  ...
                  <div className="flex-1 p-6 md:p-8 flex flex-col justify-center">
                    ...
                    <Link href={`/blog/${post.id}`}>
                      <h3 className="text-[24px] md:text-[28px] font-bold text-navy leading-tight hover:text-accent transition-colors mb-3 line-clamp-2">{post.title}</h3>
                    </Link>
          ...
          <div className="lg:col-span-4 space-y-10">
            ...
            <div className="bg-gray-50 border border-gray-200 p-6">
              <h3 className="text-[18px] font-bold text-navy mb-5 border-b-2 border-accent pb-2 inline-block">Search here</h3>
              <div className="relative mt-4">
                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400"><Search className="h-4 w-4" /></div>
                <input type="text" placeholder="Search..." className="w-full pl-10 pr-4 py-3 bg-white border border-gray-200 focus:border-accent outline-none text-[14px] text-navy placeholder:text-gray-400 transition-colors" />
              </div>
            </div>

            {/* Recent Posts */}
            <div className="bg-gray-50 border border-gray-200 p-6">
              <h3 className="text-[18px] font-bold text-navy mb-5 border-b-2 border-accent pb-2 inline-block">Recent Post</h3>
                ...
                {blogPosts.slice(0, 3).map((post) => (
                  <Link key={post.id} href={`/blog/${post.id}`} className="flex items-center gap-4 group">
                    ...
                    <div className="flex-1">
                      <h4 className="text-[14px] font-semibold text-navy group-hover:text-accent transition-colors line-clamp-2 leading-snug">{post.title.length > 30 ? post.title.substring(0, 30) + "..." : post.title}</h4>
            ...
            {/* Categories */}
            <div className="bg-gray-50 border border-gray-200 p-6">
              <h3 className="text-[18px] font-bold text-navy mb-5 border-b-2 border-accent pb-2 inline-block">Categories</h3>
              <ul className="space-y-2.5 text-[14px] font-medium">
                {["Branding", "Business", "Consulting", "Innovations", "Managements", "Marketing"].map((cat, idx) => (
                  <li key={cat}><Link href="#" className="flex justify-between items-center text-navy hover:text-accent transition-colors bg-white border border-gray-200 px-4 py-3 hover:border-accent"><span>{cat}</span><span className="text-gray-500 font-normal">({idx + 1})</span></Link></li>
                ))}
              </ul>
            </div>
            ...
            {/* Tags */}
            <div className="bg-gray-50 border border-gray-200 p-6">
              <h3 className="text-[18px] font-bold text-navy mb-5 border-b-2 border-accent pb-2 inline-block">Tags</h3>
              <div className="flex flex-wrap gap-2.5 mt-2">
                {["Branding", "Business", "Design", "Marketing", "Strategy", "AI", "Automation"].map((tag) => (
                  <Link key={tag} href="#" className="px-4 py-1.5 bg-white border border-gray-200 rounded-full text-[13px] font-medium text-navy hover:bg-navy hover:text-white transition-colors">{tag}</Link>
                ))}
              </div>
            </div>
