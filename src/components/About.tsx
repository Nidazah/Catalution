...
export default function AboutSection({
  ...
  const aboutImage =
    image && image.trim() !== "" ? image : "/images/about/h5-about-1.webp";
  const aboutEyebrow =
    eyebrow && eyebrow.trim() !== "" ? eyebrow : "ABOUT OUR COMPANY";
  ...
  return (
    <section className="relative w-full overflow-hidden bg-section pt-0 pb-10 sm:pb-12 lg:pb-14">
      ...
      <div className="relative mx-auto flex w-full justify-center">
        <div className="cms-layout-content-image relative mx-auto grid w-full max-w-[1020px] grid-cols-1 items-center justify-items-center gap-14 px-6 md:grid-cols-[390px_1fr] md:gap-[68px] lg:px-0">
          <motion.div
            className="cms-layout-role-image relative mx-auto w-full max-w-[390px]"
            ...
            <div className="relative h-[450px] w-full overflow-hidden rounded-[3px] md:h-[420px]">
              ...
              <div className="absolute bottom-7 left-4">
                <p className="h5 text-white font-light">
                  {settings?.statBadgeLabel || "Reach"}{" "}
                  <span className="font-bold text-white">
                    {settings?.statBadgeValue || "20M"}
                  </span>
                </p>
              </div>
            </div>
          ...
          <motion.div
            className="cms-layout-role-content relative mx-auto w-full max-w-[460px] text-left"
            variants={container}
            initial="hidden"
            animate="show"
          >
            ...
            <motion.p
              variants={item}
              className="body-md mt-4 max-w-[420px] text-heading"
            >
              {aboutDescription}
            </motion.p>
            ...
            >
              <div className="flex flex-1 flex-col justify-center px-[15px]">
                <p className="h5 font-bold text-navy">
                  {settings?.stat1Value || "8.5x"}
                </p>
                <p className="ui-xs mt-[4px] font-medium text-heading">
                  {settings?.stat1Label || "Faster growth"}
                </p>
              </div>
              ...
              <div className="relative flex flex-1 flex-col justify-center px-[15px]">
                ...
                <p className="h5 font-bold text-navy">
                  {settings?.stat2Value || "20M"}
                </p>
                <p className="ui-xs mt-[4px] font-medium text-heading">
                  {settings?.stat2Label || "Reach worldwide"}
                </p>
              </div>
            </motion.div>
