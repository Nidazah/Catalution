...
export default function CaseStudy({ eyebrow, title, description, items }: CaseStudyProps) {
  const [fallbackProjects, setFallbackProjects] = useState<
    Array<{ id: string; title: string; img: string; tags: string[]; link: string }>
  >([]);
    ...
    fetch("/api/portfolio", { cache: "no-store" })
      ...
      .then((data) => {
        ...
        const published = data.filter(
          (p: ApiPortfolio) => p.published === undefined || p.published === true,
        );

        const mapped = published.map((p: ApiPortfolio) => ({
          id: p.id,
          title: p.title,
        ...
        if (!cancelled && mapped.length > 0) setFallbackProjects(mapped);
      })
      .catch(() => undefined);
  ...
  const projects = items && items.length > 0
    ? items.map((item, index) => ({
        ...
        tags: Array.isArray(item.tags) && item.tags.length ? item.tags : item.meta ? [item.meta] : ["Case Study"],
