...
const pagesLinks = {
  ...
  other: [
    ...
    { label: "Service details", href: "/services/1", badge: null }, // Connects to your dynamic [id] folder
    { label: "Portfolios", href: "/portfolios", badge: null },
    { label: "Portfolio details", href: "/portfolios/1", badge: null }, // Connects to your dynamic [id] folder
    { label: "Blog grid", href: "/blog-grid", badge: "NEW" }, // Connects to your blog-grid folder
    { label: "Blog standard", href: "/blog", badge: null }, // Connects to your main blog folder
    { label: "Blog sidebar", href: "/blog-sidebar", badge: null }, // Connects to your blog-sidebar folder
    { label: "Blog details", href: "/blog/1", badge: null }, // Connects to your dynamic [id] folder
  ],
};
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          /* 
            ALIGNMENT FIX: 
            Instead of 'fixed left-1/2 -translate-x-1/2', we use 'absolute' 
            with 'left-0' and 'w-[760px]'.
            This ensures the left edge of the dropdown perfectly aligns 
            with the left edge of the 'Pages' button.
          */
...
function NavItem({ link, pathname }: { link: NavLink; pathname: string }) {
  ...
  if (link.isMegaMenu) {
    return (
      ...
      >
        <button
          className={`relative text-[15px] font-semibold transition-colors inline-flex items-center gap-1 pb-1 ${open ? "text-[var(--color-accent)]" : "text-[var(--color-heading)] hover:text-[var(--color-accent)]"}`}
        >
          {link.label}
          <ChevronDown
  ...
  if (!link.dropdown) {
    return (
      <Link
        href={link.href}
        className={`text-[15px] font-semibold transition-colors inline-flex items-center gap-1 ${isActive ? "text-[var(--color-accent)]" : "text-[var(--color-heading)] hover:text-[var(--color-accent)]"}`}
      >
        {link.label}
      </Link>
    );
  ...
  return (
    ...
    >
      <Link
        href={link.href}
        className={`relative text-[15px] font-semibold transition-colors inline-flex items-center gap-1 pb-1 ${open || isActive ? "text-[var(--color-accent)]" : "text-[var(--color-heading)] hover:text-[var(--color-accent)]"}`}
      >
        {link.label}
        <ChevronDown
          className={`h-3.5 w-3.5 transition-transform duration-200 ${open ? "rotate-180" : ""}`}
          ...
          >
            {link.dropdown.map((item) => (
              <Link
                ...
                href={item.href}
                data-cursor-hover
                className="block px-5 py-2.5 text-[15px] font-semibold text-[#0B1426] transition-colors hover:text-[var(--color-accent)]"
              >
                {item.label}
              </Link>
            ))}
...
function MobileSection({
  ...
  if (!link.dropdown && !link.isMegaMenu) {
    return (
      <Link
        href={link.href}
        onClick={onNavigate}
        className={`text-sm font-semibold ${isActive ? "text-[var(--color-accent)]" : "text-[var(--color-heading)]"}`}
      >
        {link.label}
      </Link>
    );
  ...
  return (
    <div>
      <button
        onClick={() => setOpen((v) => !v)}
        className={`flex w-full items-center justify-between text-sm font-semibold ${isActive ? "text-[var(--color-accent)]" : "text-[var(--color-heading)]"}`}
      >
        {link.label}
        <ChevronDown
          className={`h-3.5 w-3.5 transition-transform duration-200 ${open ? "rotate-180" : ""}`}
        ...
        <div className="mt-3 ml-3 flex flex-col gap-3 border-l border-[var(--color-line)] pl-3">
          {link.dropdown.map((item) => (
            <Link
              ...
              href={item.href}
              onClick={onNavigate}
              className="text-sm text-[var(--color-body)] hover:text-[var(--color-accent)]"
            >
              {item.label}
            </Link>
          ))}
...
export default function Navbar() {
  ...
  return (
    <motion.header
      ...
      className="fixed top-0 inset-x-0 z-50 bg-white/90 backdrop-blur-md border-b border-[var(--color-line)]"
    >
      <div className="mx-auto max-w-7xl px-6 h-20 flex items-center justify-between">
        <Link
          href="/"
          className="flex items-center gap-2 font-display font-bold text-2xl text-[var(--color-heading)]"
        >
        ...
        <nav className="hidden lg:flex items-center gap-9">
          {links.map((l) => (
            <NavItem key={l.label} link={l} pathname={pathname} />
          ))}
        </nav>

        <div className="hidden md:flex items-center gap-6">
          <button className="inline-flex items-center gap-2 text-sm font-semibold text-[var(--color-heading)]">
          ...
          <motion.a
            ...
            className="group inline-flex items-center gap-2 rounded-full bg-[var(--color-navy)] pl-2 pr-6 py-2 text-sm font-semibold text-white"
          >
