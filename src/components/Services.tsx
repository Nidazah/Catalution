...
const services = [
  {
    ...
    icon: (
      <svg viewBox="0 0 48 48" fill="none" className="h-12 w-12 text-white/80">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4">
          {services.map((s, i) => (
            <ScrollReveal key={s.title} delay={i * 0.08}>
              <div
                className={`
                  group relative h-full overflow-hidden p-8 md:p-12 flex flex-col items-center text-center transition-all duration-300 cursor-pointer
                  ${i !== services.length - 1 ? "md:border-r md:border-white/10" : ""}
                `}
              >
