...
interface TestimonialsProps { eyebrow?: string; title?: string; cmsSettings?: { quotes?: Array<{text:string;name:string;role:string;avatar?:string}> }; }
...
export default function Testimonials({ eyebrow, title, cmsSettings }: TestimonialsProps) {
  const [quotes, setQuotes] = useState<Array<{
    text: string;
    name: string;
    role: string;
    avatar?: string;
  }>>(cmsSettings?.quotes?.length ? cmsSettings.quotes : fallbackQuotes);
  ...
  useEffect(() => {
    let cancelled = false;
    if (cmsSettings?.quotes?.length) return;
    fetch("/api/testimonials", { cache: "no-store" })
      .then((res) => (res.ok ? res.json() : Promise.reject(new Error("Failed to load testimonials"))))
      .then((data) => {
        if (!cancelled && Array.isArray(data) && data.length > 0) {
          setQuotes(data.map((item) => ({ text: item.quote, name: item.name, role: item.role, avatar: item.avatar || undefined })));
        }
      })
      .catch(() => undefined);
                ...
                >
                  ...
                  <figcaption className="mt-6 flex items-center gap-3">
                    <div className="h-9 w-9 rounded-full bg-[var(--color-navy)] flex items-center justify-center font-display text-xs font-bold text-white shrink-0 overflow-hidden">
                      {q.avatar ? <img src={q.avatar} alt="" className="h-full w-full object-cover" /> : q.name.split(" ").map((n) => n[0]).join("")}
                    </div>
